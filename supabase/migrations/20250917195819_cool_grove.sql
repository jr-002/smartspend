/*
  ...
  This migration establishes a clean, consistent database schema with proper
  RLS policies, triggers, and data integrity constraints.
  
  1. Schema Setup
     - Create/update notification enums
     - Create utility functions for triggers
     - Ensure all tables exist with proper structure
  
  2. Security
     - Enable RLS on all tables
     - Create consistent policies for all operations
...
EXCEPTION
    WHEN unique_violation THEN
        ...
        SET 
            ...
            monthly_income = COALESCE((NEW.raw_user_meta_data->>'monthly_income')::numeric, monthly_income),
            updated_at = now()
        WHERE id = NEW.id;
        RETURN NEW;
END;
...
BEGIN
    ...
    RAISE NOTICE 'Schema fix complete. Tables: %, Policies: %, Triggers: %', table_count, policy_count, trigger_count;
END $$;